...
import { askGemini } from './gemini-client.js'; // Precisaremos do Gemini para resumir
...
function getHistoryFilePath(courseId) {
    return path.join(HISTORY_DIR, `course_${courseId}_history.json`);
}

async function getHistory(courseId) {
    // ... (sem grandes alterações, talvez logar se o ficheiro não existe)
    await ensureDirectoryExists();
    const historyFilePath = getHistoryFilePath(courseId);
    try {
        const data = await fs.readFile(historyFilePath, 'utf8');
        return JSON.parse(data);
    } catch (error) {
        if (error.code === 'ENOENT') return [];
        console.error(`Erro ao ler histórico do curso ${courseId}:`, error);
        return [];
    }
}

async function saveHistory(courseId, history) {
    await ensureDirectoryExists();
    const historyFilePath = getHistoryFilePath(courseId);
    try {
        await fs.writeFile(historyFilePath, JSON.stringify(history, null, 2), 'utf8');
        console.log(`Histórico salvo para o curso ${courseId} (${history.length} entradas totais)`);
    } catch (error) {
...
async function summarizeOldInteractions(courseId, interactionsToSummarize) {
    ...
    let textToSummarize = "Resuma os pontos chave das seguintes interações de uma conversa entre um Professor e um Assistente de IA sobre um curso Moodle:\n\n";
...
Resumo conciso:`;
    ...
    try {
        console.log(`Solicitando resumo de ${interactionsToSummarize.length} interações para o curso ${courseId}`);
...
async function addToHistory(courseId, question, answer) {
    ...
    if (history.length > HISTORY_SUMMARY_THRESHOLD) {
        ...
        const actualInteractions = history.filter(item => item.type === 'interaction');
        
        if (actualInteractions.length >= HISTORY_SUMMARY_THRESHOLD) {
            const interactionsToSummarize = actualInteractions.slice(0, ENTRIES_TO_SUMMARIZE);
            const remainingInteractions = actualInteractions.slice(ENTRIES_TO_SUMMARIZE);
            
            // Obter resumos anteriores, se houver
            const previousSummaries = history.filter(item => item.type === 'summary');

            if (interactionsToSummarize.length > 0) {
                const summaryText = await summarizeOldInteractions(courseId, interactionsToSummarize);
                const summaryEntry = {
                    type: 'summary',
                ...
                history = [...previousSummaries, summaryEntry, ...remainingInteractions];
                console.log(`Resumo de ${interactionsToSummarize.length} interações adicionado ao histórico do curso ${courseId}.`);
            }
        }
    }
    ...
    if (history.length > MAX_RAW_HISTORY_ENTRIES) {
        ...
        let summariesToRemove = Math.min(excess - interactionsToRemove, summaries.length);

        const keptInteractions = interactions.slice(interactionsToRemove);
        const keptSummaries = summaries.slice(summariesToRemove);
        
        history = [...keptSummaries, ...keptInteractions].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    }
...
async function formatHistoryForLLM(courseId) {
    const history = await getHistory(courseId);
    if (history.length === 0) {
    ...
    for (let i = history.length - 1; i >= 0 && entriesForContext < MAX_CONTEXT_HISTORY_ENTRIES; i--) {
    ...
    if (entriesForContext === 0)  return "Não há histórico de conversas anteriores para este curso.";
    return output;
}

export { addToHistory, getHistory, formatHistoryForLLM }; // getHistory pode ser útil para debug